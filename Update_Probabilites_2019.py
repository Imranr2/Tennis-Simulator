...
def callPAT(patPath):
  patOutputFile =  "./result_2019.txt"
  
  for model in ["APPROACH", "BASELINE"]:
    mdpPredFile = f"./MDP_pred_2019.csv"

    f = open(patOutputFile, "w")
    f.close()
    mdpPredLines = [line for line in csv.reader(open(mdpPredFile, 'r'))]
    newMdpPredFp = open(f"./MDP_pred_with_prob/2019/MDP_pred_new_{model}.csv", 'w')
    ...
    for i in range(1, len(mdpPredLines)):
      args = [patPath + "/PAT3.Console.exe", "-pcsp", f"./pcsp_files_2019/{i}_{model}.pcsp", os.getcwd() + patOutputFile]
