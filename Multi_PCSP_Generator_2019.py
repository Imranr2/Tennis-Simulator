...
with open('MDP_pred_2019.csv', newline='') as pred_file:
    ...
    for i, row in enumerate(pred_reader):
        ...
        generate_pcsp_approach(
            ...
            row['P1Hand'], row['P2Hand'], "./pcsp_files_2019/" + f"{i + 1}_APPROACH.pcsp"
        )

        generate_pcsp_baseline(
            data, end_date.strftime('%Y-%m-%d'), row['P1Name'], row['P2Name'], 
            row['P1Hand'], row['P2Hand'], "./pcsp_files_2019/" + f"{i + 1}_BASELINE.pcsp"
        )
