...
def callPAT(patPath):
  patOutputFile =  "./result_2020.txt"
  
  for model in ["APPROACH", "BASELINE"]:
    mdpPredFile = f"./MDP_pred_2020.csv"

    f = open(patOutputFile, "w")
    f.close()
    mdpPredLines = [line for line in csv.reader(open(mdpPredFile, 'r'))]
    newMdpPredFp = open(f"./MDP_pred_with_prob/2020/MDP_pred_new_{model}.csv", 'w')
    ...
    for i in range(1, len(mdpPredLines)):
      args = [patPath + "/PAT3.Console.exe", "-pcsp", f"./pcsp_files_2020/{i}_{model}.pcsp", os.getcwd() + patOutputFile]
