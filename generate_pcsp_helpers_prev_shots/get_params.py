...
def get_params_helper(df, hand):
    ...
    for i, Stroke in enumerate([De_Stroke, Mid_Stroke, Ad_Stroke]):
        FH_Stroke_Given_FH_Stroke = Stroke.query('shot<=20 and prev_shot<=20')
        # FH_Stroke_Given_FH_Stroke = Stroke.query('shot<=20 and prev_shot<=20 and prev_shot_type==3')
        FH_Stroke_Given_BH_Stroke = Stroke.query('shot<=20 and prev_shot>20')
        BH_Stroke_Given_BH_Stroke = Stroke.query('shot<=40 and shot>20 and prev_shot>20')
        BH_Stroke_Given_FH_Stroke = Stroke.query('shot<=40 and shot>20 and prev_shot<=20')
        FH_FH_shots = [FH_Stroke_Given_FH_Stroke.query('to_which_court==@to_dir') for to_dir in directions[i][0]]
        FH_BH_shots = [FH_Stroke_Given_BH_Stroke.query('to_which_court==@to_dir') for to_dir in directions[i][0]]
        BH_BH_shots = [BH_Stroke_Given_BH_Stroke.query('to_which_court==@to_dir') for to_dir in directions[i][1]]
        BH_FH_shots = [BH_Stroke_Given_FH_Stroke.query('to_which_court==@to_dir') for to_dir in directions[i][1]]

        # shots = FH_shots + BH_shots
        FH_FH_stroke_in = [len(x.query('shot_outcome==7')) for x in FH_FH_shots]
        FH_BH_stroke_in = [len(x.query('shot_outcome==7')) for x in FH_BH_shots]
        BH_BH_stroke_in = [len(x.query('shot_outcome==7')) for x in BH_BH_shots]
        BH_FH_stroke_in = [len(x.query('shot_outcome==7')) for x in BH_FH_shots]
        stroke_win = [len(Stroke.query('shot_outcome in [1, 5, 6]'))]
        stroke_err = [len(Stroke.query('shot_outcome in [2, 3, 4]'))]
        results.append(FH_FH_stroke_in + FH_BH_stroke_in + BH_BH_stroke_in + BH_FH_stroke_in + stroke_win + stroke_err)
